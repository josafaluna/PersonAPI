...
public class PersonUtils {
	...
	public static PersonDTO createFakeDTO() {
		return PersonDTO.builder().fisrtName(FIRST_NAME).lastName(LAST_NAME).cpf(CPF_NUMBER).birthDate("04-04-2010")
				.phones(Collections.singletonList(PhoneUtils.createFakeDTO())).build();
	}

	public static Person createFakeEntity() {
		return Person.builder().id(PERSON_ID).fisrtName(FIRST_NAME).lastName(LAST_NAME).cpf(CPF_NUMBER)
